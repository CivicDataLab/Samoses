...
function coverage_details() {
  ...
  for($i=0;$i<count($data);$i++) {
    list($word,$count) = explode("\t",$data[$i]);
    $item["word"] = $word;
...
function segmentation_summary() {
  ...
  if (file_exists($file)) {
    ...
    for($i=0;$i<count($data);$i++) {
      ...
      if ($type == "rule") {
        list($rule_in,$in,$nt,$rule_out,$out) = explode(":",$rule);
        if ($by == "word") { $c *= $in; }
        if ($in>4)  { $in  = 4; }
        if ($out>4) { $out = 4; }
        $total += $c;
...
function rule_summary() {
  ...
  foreach ($data as $item) {
	...
	list($rule_in,$word_in,$nt,$rule_out,$word_out) = explode(":",$d);
...
function sentence_annotation($count,$filter) {
  ...
  if (file_exists($file)) {
	...
	$segment = 0;
	foreach (explode(" ",$data[$i]) as $item) {
	  list($in_start,$in_end,$out_start,$out_end) = explode(":",$item);
	  $segment++;
  ...
  if (file_exists($file)) {
    ...
    $nt_count = array();
    for($i=0;$i<count($data);$i++) {
      list($sentence,$brackets,$nt,$words) = explode("\t",$data[$i]);
...
#     }
  ...
  if (file_exists($file)) {
    ...
    $last_sentence = -1;
    for($i=0;$i<count($data);$i++) {
      list($sentence,$depth,$start_div,$end_div,$start_div_in,$end_div_in,$children) = explode(" ",$data[$i]);
...
function coverage($coverage_vector) {
  ...
  $coverage = array();
  foreach (explode(" ",$coverage_vector) as $item) {
    if (preg_match("/[\-:]/",$item)) {
...
function input_annotation($sentence,$input,$segmentation,$filter) {
  global $biconcor;
  list($words,$coverage_vector) = explode("\t",$input);
...
function output_annotation($sentence,$system,$segmentation) {
  ...
  for($j=0;$j<count($word);$j++) {
    list($surface,$correct) = explode("\|", $word[$j]);
...
function annotation_hierarchical($sentence,$segmentation,$segmentation_out,$node) {
 ...
 function sentence_annotation_hierarchical($info,$sentence,$sequence,$segmentation,$in_out) {
	...
	$span_word = array();
	if ($words != "") { $span_word = explode(" ",$words); }
	for($w=0;$w<count($span_word);$w++) {
		...
		#print "</span>";
	      ...
	      list($surface,$correct) = explode("\|", $word[$word_count]);
