...
function bleu_diff_annotation() {
  ...
  for($i=0;$i<count($input_annotation);$i++) {
    $item = explode("\t",$input_annotation[$i]);
    $input[$i] = $item[0];
  }
...
//  print "identical: $identical (%d), same: $same, better: $better, worse: $worse<br>\n";
  ...
  for($i=0;$i<$count && $i<count($annotation);$i++) {
     ...
     $word_with_score1 = explode(" ",$line["system1"]);
     $word_with_score0 = explode(" ",$line["system0"]);
     $word1 = explode(" ",preg_replace("/\|\d/","",$line["system1"]));
     $word0 = explode(" ",preg_replace("/\|\d/","",$line["system0"]));
